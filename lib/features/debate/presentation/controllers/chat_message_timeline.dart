...
class ChatMessageTimeline extends ChangeNotifier {
  ChatMessageTimeline({
    Iterable<CommunityChatMessage> initialMessages = const [],
  ...
  bool get hasPendingMessages => _messages.any(
    (message) =>
        message.deliveryStatus == CommunityChatMessageDeliveryStatus.pending,
  );

  List<CommunityChatMessage> get orderedMessages {
    return [..._messages]
      ..sort((left, right) => left.createdAt.compareTo(right.createdAt));
  }

  void addPending(CommunityChatMessage message) {
  ...
  void applyEvent(CommunityChatEvent event) {
    switch (event.type) {
      case CommunityChatEventType.messageCreated:
      case CommunityChatEventType.messageUpdated:
      case CommunityChatEventType.opinionSubmitted:
      ...
      case CommunityChatEventType.debateStarted:
      case CommunityChatEventType.debateEnded:
        return;
    }
  }

  void upsert(CommunityChatMessage incoming) {
